...
public class ProductDao {
    ...
    public List<GetProductRes> getProductsByProductName(String productName) {
        String getProductsByProductNameQuery = "select * from Product where productName =?"; // 해당 닉네임을 만족하는 유저를 조회하는 쿼리문
    ...
    public int createProduct(PostProductReq postProductReq) {
        String createProductQuery = "insert into Product (userIdx,categoryIdx ,productName,price,productDetail) VALUES (?,?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createProductParams = new Object[]{postProductReq.getUserIdx(),postProductReq.getCategoryIdx(), postProductReq.getProductName(), postProductReq.getPrice(), postProductReq.getProductDetail()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int modifyProductName(PatchProductReq patchProductReq) {
        String modifyProductNameQuery = "update Product set productName = ? where productIdx = ? "; // 해당 userIdx를 만족하는 User를 해당 nickname으로 변경한다.
        Object[] modifyProductNameParams = new Object[]{patchProductReq.getProductName(), patchProductReq.getProductIdx()}; // 주입될 값들(nickname, userIdx) 순

...
public class ProductController {
    ...
    /**
     * 모든 제품들  조회 API (paging처리)
     * [GET] /products/paging?pageNo= &pageSize=
     *
     *
     */
    ...
    @ResponseBody   // return되는 자바 객체를 JSON으로 바꿔서 HTTP body에 담는 어노테이션.
    //  JSON은 HTTP 통신 시, 데이터를 주고받을 때 많이 쓰이는 데이터 포맷.
    @GetMapping("paging") // (GET) 127.0.0.1:9000/app/products/paging
    // GET 방식의 요청을 매핑하기 위한 어노테이션
    public BaseResponse<List<GetProductRes>> getProductsPaging(@RequestParam(required = false, defaultValue = "-1") int pageNo,
                                                               @RequestParam(required = false, defaultValue = "-1") int pageSize) {
        //  @RequestParam은, 1개의 HTTP Request 파라미터를 받을 수 있는 어노테이션(?뒤의 값). default로 RequestParam은 반드시 값이 존재해야 하도록 설정되어 있지만, (전송 안되면 400 Error 유발)
        //  지금 예시와 같이 required 설정으로 필수 값에서 제외 시킬 수 있음
        //  defaultValue를 통해, 기본값(파라미터가 없는 경우, 해당 파라미터의 기본값 설정)을 지정할 수 있음
        try {
            if (pageNo == -1 || pageSize== -1) { // query string인 pageNo,pageSize가 없을 경우, 그냥 전체 제품정보를 불러온다.
                throw new BaseException(EMPTY_PAGENO_OR_PAGESIZE);
            }
            // query string인 pageNo,pageSize가 있을 경우, 조건을 만족하는 유저정보들을 불러온다.
            List<GetProductRes> getProductsRes = productProvider.getProductsPaging(pageNo,pageSize);
            return new BaseResponse<>(getProductsRes);
        } catch (BaseException exception) {
            return new BaseResponse<>((exception.getStatus()));
        }

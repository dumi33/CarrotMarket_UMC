...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (nickname,email, password, address) VALUES (?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getNickname(),postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getAddress()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public List<GetUserRes> getUsersByAddress(String address) {
        ...
        return this.jdbcTemplate.query(getUsersByAddressQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("password"),
                        rs.getString("address")), // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
                getUsersByAddressParams); // 해당 주소을 갖는 모든 User 정보를 얻기 위해 jdbcTemplate 함수(Query, 객체 매핑 정보, Params)의 결과 반환
    }
    // 해당 userIdx를 갖는 유저조회
